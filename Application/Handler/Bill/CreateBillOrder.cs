    ...
    public class CreateBillOrder
    {
        ...
        public class Handler : IRequestHandler<Query, Result<ActionResult>>
        {
            ...
            public async Task<Result<ActionResult>> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                {
                    return Result<ActionResult>.Failure("Order không tồn tại.");
                }

                var booking = await _context.Bookings
                    .Include(b => b.Court)
                    .FirstOrDefaultAsync(b => b.Id == orders[0].BookingId, cancellationToken);

                if (booking == null)
                {
                    return Result<ActionResult>.Failure("Booking không tồn tại.");
                }

                var court = await _context.Courts
                    .FirstOrDefaultAsync(c => c.Id == booking.Court.Id, cancellationToken);

                if (court == null)
                {
                    return Result<ActionResult>.Failure("Court không tồn tại.");
                }

                var courtCluster = await _context.CourtClusters
                    .FirstOrDefaultAsync(cc => cc.Id == court.CourtClusterId, cancellationToken);

                if (courtCluster == null)
                {
                    return Result<ActionResult>.Failure("Court cluster không tồn tại.");
                }
                ...
                foreach (var order in orders)
                {
                    foreach (var orderDetail in order.OrderDetails)
                    {
                        ...
                        {
                            var existingProduct = productBillDtos.FirstOrDefault(p => p.ProductId == orderDetail.ProductId);
                            if (existingProduct != null)
                            {
                                existingProduct.Quantity += (decimal)orderDetail.Quantity;
                            }
                            else
                            {
                        ...
                        {
                            var existingService = serviceBillDtos.FirstOrDefault(s => s.ServiceId == orderDetail.ServiceId);
                            if (existingService != null)
                            {
                                existingService.Quantity += (decimal)orderDetail.Quantity;
                            }
                            else
                            {
