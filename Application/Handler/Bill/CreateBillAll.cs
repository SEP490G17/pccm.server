...
namespace Application.Handler.Bill
{
    public class CreateBillAll
    {
        ...
        public class Handler : IRequestHandler<Query, Result<ActionResult>>
        {
            ...
            public Handler(DataContext context, IMapper mapper, IConverter converter)
            {
            ...
            public async Task<Result<ActionResult>> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                var globalSettings = new GlobalSettings
                {
                    ...
                    Margins = new MarginSettings { Left = 10, Right = 10, Bottom = 10 }
                };

                var objectSettings = new ObjectSettings
                {
                    HtmlContent = GenerateHtml(billData),
                ...
                return Result<ActionResult>.Success(new FileContentResult(file, "application/pdf"));
            }
        }
            ...
            <div class='details'>
                <table style='width: 100%; border-collapse: collapse;'>
                    <tr>
                        ...
                        <td style='text-align: right;'>{data.booking.StartTime.ToString("HH:mm dd/MM/yyyy")}</td>
                    </tr>
                    <tr>
                        <td style='text-align: left;'>Giờ kết thúc</td>
                        <td style='text-align: right;'>{data.booking.EndTime.ToString("HH:mm dd/MM/yyyy")}</td>
                    </tr>
                </table>
            </div>
            <table class='items'>
            ...
            <div class='totals' style='margin-top: 20px;'>
                <table style='width: 100%; border-collapse: collapse;'>
                    <tr>
                        ...
                        <td style='text-align: right;'>{(totalProduct + totalService).ToString("N0")}</td>
                    </tr>
                    <tr>
                        <td style='text-align: left;'>Tổng tiền giờ</td>
                        <td style='text-align: right;'>{data.booking.TotalPrice.ToString("N0")}</td>
                    </tr>
                    <tr style='height: 15px;'></tr>
                    <tr>
                        <td style='text-align: left; font-weight: bold;'>Thanh toán</td>
                        <td style='text-align: right; font-weight: bold;'>{(totalProduct + totalService + data.booking.TotalPrice).ToString("N0")}</td>
                    </tr>
                </table>
            </div>
        </div>
